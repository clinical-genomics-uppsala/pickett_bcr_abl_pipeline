...
rule dotnet_pisces:
    ...
    output:
        vcf=temp(
            "snv_indels/pisces/{sample}_{type}_{chr}_bad_name/{sample}_{type}_{chr}.vcf"
        ),
        pisces_log=temp(
            "snv_indels/pisces/{sample}_{type}_{chr}_bad_name/PiscesLogs/PiscesLog.txt"
        ),
        pisces_options=temp(
            "snv_indels/pisces/{sample}_{type}_{chr}_bad_name/PiscesLogs/PiscesOptions.used.json"
        ),
    log:
    ...
    resources:
        mem_mb=config.get("dotnet_pisces", {}).get(
            "mem_mb", config["default_resources"]["mem_mb"]
        ),
        mem_per_cpu=config.get("dotnet_pisces", {}).get(
            "mem_per_cpu", config["default_resources"]["mem_per_cpu"]
        ),
        partition=config.get("dotnet_pisces", {}).get(
            "partition", config["default_resources"]["partition"]
        ),
        threads=config.get("dotnet_pisces", {}).get(
            "threads", config["default_resources"]["threads"]
        ),
        time=config.get("dotnet_pisces", {}).get(
            "time", config["default_resources"]["time"]
        ),
    params:
        extra=config.get("dotnet_pisces", {}).get(
            "extra", "--gvcf FALSE --filterduplicates TRUE"
        ),
    message:
